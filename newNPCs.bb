...
Type NPCs
	...
	Field Task.Tasks[MAXTASKS], SelectedTask.Tasks
End Type
...
Global UpdateNPCsTimer%

Function UpdateNPCsSlow()
	If UpdateNPCsTimer > MilliSecs() Then Return
	
	Local n.NPCs, n2.NPCs, npc.NPCs, npc2.NPCs
	
	For npc.NPCs = Each NPCs
		For i = 0 To MAXTASKS-1
			If npc\Task[i]<>Null Then 
				If npc\Task[i]\Priority =< 0 Then
					RemoveTask(npc\Task[i], npc)
				ElseIf npc\Task[i]\Name = "attack"
					RemoveTask(npc\Task[i], npc)
				EndIf
			EndIf
		Next
		...
		Select npc\NPCtype
			Case NPCtype173
				;If CheckVisibility(n, Null) Then AddTask(n, "attack", 1000-EntityDistance(Collider,npc\Collider)*10, 0, Null)
				If EntityDistance(Collider,npc\Collider)<8.0 Then AddTask(npc, "attack", 1000-EntityDistance(Collider,npc\Collider)*20, 0, Null)
				;AddTask(n, "search", 700-EntityDistance(Collider,npc\Collider)*10, 10, Null)
				AddTask(npc, "roam", 10, 0, Null)
			Case NPCtypeOldMan
				If npc\State <= 0 Then
					AddTask(npc,"attack",1000-EntityDistance(Collider,npc\Collider)*20,0,Null)
				Else
					AddTask(npc,"idle",10,0,Null)
				EndIf
			;Case NPCtypeOldMan
			;Case NPCtype096
			;Case NPCtypeMTF
		...
		npc2=After(npc)
		While npc2<>Null
			If npc\NPCtype < npc2\NPCtype Then
				n=npc
				n2=npc2
			Else
				n=npc2
	...
	UpdateNPCsTimer = MilliSecs()+1000
End Function

Function UpdateNPCs()
	Local n.NPCs, n2.NPCs, d.Doors, de.Decals, r.Rooms
	Local i%, dist#, dist2#, angle#, x#, y#, z#, prevframe#, temp#
	...
	For n.NPCs = Each NPCs
		...
		For i = 0 To MAXTASKS 
			If n\Task[i]<>Null Then
				If n\Task[i]\TargetType = "npc" Then
					If n\Task[i]\TargetNPC=Null Then RemoveTask(n\Task[i],n)
				EndIf
			EndIf
		Next
		...
		For i = 0 To MAXTASKS 
			If n\Task[i]<>Null Then
				If n\Task[i]\Priority > temp Then
					temp = n\Task[i]\Priority
					n\SelectedTask = n\Task[i]
				EndIf
			EndIf
		Next
		...
		Select n\NPCtype
			Case NPCtype173
				...
				;move 173 closer to the player if it's too far away
				If Rand(500)=1 Then
				...
				;True = no-one is looking, false = someone is looking, stop moving
				temp = True
				If dist < 16.0 Then
					...
					If dist < 10.0 Then 
						...
						If EntityInView(n\obj, Camera) And (BlinkTimer > - 6 Or BlinkTimer < - 16) Then
							temp = False
				...
				If n\SelectedTask <> Null Then
					Select n\SelectedTask\Name
						Case "attack"
							If temp Then ;no-one is looking
								...
								If n\SelectedTask\TargetType="player" Then
									If dist < 0.65 Then
							...
							If n\PathStatus = 1 Then
								...
								If n\Path[n\PathLocation]=Null Then 
									If n\PathLocation > 19 Then 
										n\PathLocation = 0
										n\PathStatus = 0
										If n\LastSeen > 0 Then RemoveTask(n\SelectedTask, n) : n\PathTimer = 0 ;etsimässä pelaajaa, palataan takaisin "hyökkäysmoodiin"
									Else
										n\PathLocation = n\PathLocation + 1
									EndIf
								Else
				...
				Case NPCtype173
					...
					Else
						...
						If n\Idle = False Then
							...
							SoundVol# = Max(Min((Distance(EntityX(n\Collider), EntityZ(n\Collider), n\PrevX, n\PrevZ) * 2.5), 1.0), 0.0)
							n\SoundChn = LoopSound2(StoneDragSFX, n\SoundChn, Camera, n\Collider, 10.0, n\State)
...
Function CheckVisibility(n.NPCs, n2.NPCs)
	If n2=Null Then
		If Abs(EntityX(n\Collider)-EntityX(Collider))<10.0 Then
			If Abs(EntityZ(n\Collider)-EntityZ(Collider))<10.0 Then
...
Type Tasks
	Field Name$
	Field Priority#, Decrease#
	Field Value#[4]
	
	Field TargetNPC.NPCs, TargetType$
End Type 

Function AddTask.Tasks(NPC.NPCs, name$, priority#, timer%, targetNPC.NPCs, targettype$="player", value1#=0, value2#=0, value3#=0)
	;check if the NPC already has an identical task assigned to it
	For i = 0 To MAXTASKS-1
		If NPC\Task[i]<>Null Then
			If NPC\Task[i]\Name = name And NPC\Task[i]\TargetType = targettype Then
				If NPC\Task[i]\TargetNPC = targetNPC And NPC\Task[i]\Value[0]=value1 Then
					If NPC\Task[i]\Value[1]=value2 And NPC\Task[i]\Value[2]=value3 Then
						NPC\Task[i]\Priority=priority
						DebugLog "existing task: "+name+ " - "+priority
						Return NPC\Task[i]
					EndIf
				EndIf
			EndIf
	...
	Local PlaceFound%=False, Place=0
	For i = 0 To MAXTASKS-1
		If NPC\Task[i]=Null Then 
			PlaceFound = True
			Place = i
		...
		EndIf
	Next
	;no empty slot found, check if the new task has a higher priority than
	;any of the existing tasks
	Local LowestPriority = priority
	If (Not PlaceFound) Then
		For i = 0 To MAXTASKS-1
			If NPC\Task[i]\Priority < LowestPriority Then
				LowestPriority = NPC\Task[i]\Priority
				Place = i
				PlaceFound = True
			EndIf
		Next
	...
	If PlaceFound Then
		DebugLog "add task: "+name+" - "+priority+" - "+place
		
		t.tasks = New Tasks
		t\name = name
		t\priority = priority
		If timer > 0 Then t\decrease = t\priority/timer
		t\targettype = targettype
		t\targetnpc=targetNPC
		t\value[0]=value1
		t\value[1]=value2
		t\value[2]=value3
		
		NPC\Task[Place]=t
		Return NPC\Task[Place]
	EndIf
	
End Function

Function RemoveTask(t.Tasks, npc.NPCs)
	For i = 0 To MAXTASKS-1
		If npc\Task[i]=t Then npc\Task[i]=Null
	Next
	If npc\SelectedTask = t Then npc\SelectedTask=Null
	DebugLog "remove: "+t\Name
...
;~F#6#22#104#3D2#3DD#A8B#A9B#AA3#AE7#B59
;~C#Blitz3D