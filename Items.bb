...
Function InitItemTemplates()
	...
	it = CreateItemTemplate("Mysterious Note", "paper", "GFX\items\paper.x", "GFX\items\INVpaper.jpg", "GFX\items\sn.it", 0.003)
	it\sound = 0	
	
	it = CreateItemTemplate("Mobile Task Forces", "paper", "GFX\items\paper.x", "GFX\items\INVpaper.jpg", "GFX\items\docMTF.jpg", 0.003)
	it\sound = 0
	it = CreateItemTemplate("Mobile Task Force Epsilon-11", "paper", "GFX\items\paper.x", "GFX\items\INVpaper.jpg", "GFX\items\docNTF.jpg", 0.003)
	it\sound = 0	
	...
	it\sound = 0 
	it = CreateItemTemplate("Note", "paper", "GFX\items\paper.x", "GFX\items\INVpaper.jpg", "GFX\items\docRAND2.jpg", 0.003)
	it\sound = 0 
	it = CreateItemTemplate("Notification", "paper", "GFX\items\paper.x", "GFX\items\INVpaper.jpg", "GFX\items\docRAND1.jpg", 0.003)
	it\sound = 0 	
	...
	it = CreateItemTemplate("ReVision Eyedrops", "eyedrops","GFX\items\eyedrops.b3d", "GFX\items\INVeyedrops.jpg", "", 0.0012)
...
Type Items
	...
	;Field InvIMG%, IMG%
	;Field ObjPath$, InvIMGpath$, IMGpath$
	Field DropSpeed#
	...
	;Field TempName$, Name$ ;tempname = esineen ;;perustyyppi;;, name = valikoissa näkyvä nimi
	Field Picked% ;onko pelaaja poiminut esineen
End Type 
...
Function RemoveItem(i.Items, netcall% = True)
	Local n
	...
	If netcall = True
		...
		If SelectedItem = i Then
			If SelectedItem\itemtemplate\tempname = "gasmask" Or SelectedItem\itemtemplate\tempname = "supergasmask"  Then WearingGasMask = False
		...
		If mpState = 1
			For iter% = 1 To MpNumClients - 1
				WriteLine MpClients(iter%)\stream, "removeitem"
				WriteLine MpClients(iter%)\stream, offset
			Next
		EndIf
		
		If mpState = 2 Then
			WriteLine MpStream,"removeitem"
					...
					If Inventory(n) = Null Then
						Select ClosestItem\itemtemplate\tempname
							...
							Case "firstaid", "finefirstaid", "veryfinefirstaid", "firstaid2"
								ClosestItem\state = 0
							Case "navigator", "nav"
								If ClosestItem\itemtemplate\name = "S-NAV Navigator Ultimate" Then AchvSNAV = True
						End Select
						...
						Local offset% = 0
						For itemit.Items = Each Items
							If itemit = ClosestItem
								Exit
							EndIf
							offset = offset + 1
						Next
						...
						If mpState = 1
							For iter% = 1 To MpNumClients - 1
								WriteLine MpClients(iter%)\stream, "itemhide"
								WriteLine MpClients(iter%)\stream, offset
							Next
						EndIf
						
						If mpState = 2 Then
							WriteLine MpStream,"itemhide"
							WriteLine MpStream,offset
